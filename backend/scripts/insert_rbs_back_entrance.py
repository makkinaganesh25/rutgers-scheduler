...
def find_rbs_back_entrance_instances(df):
    """Locate all cells containing 'RBS Back Entrance' and return their row and column indices."""
    locations = []
    for row in range(df.shape[0]):
        for col in range(df.shape[1]):
            if df.iloc[row, col] == "RBS Back Entrance":
                locations.append((row, col))
    return locations

def extract_rbs_back_entrance_data(df, row, col):
    """Extract data for each instance of 'RBS Back Entrance'."""
    date_row = row - 1
    day_row = row
    hours_row = row + 1
    officers_row = row + 1
    ...
    return pd.DataFrame(rbs_back_entrance_data)
...
def insert_rbs_back_entrance_data(rbs_back_entrance_data_df):
    """Insert the extracted RBS Back Entrance shift data into the database."""
    for _, entry in rbs_back_entrance_data_df.iterrows():
        cursor.execute(
            """
            INSERT INTO rbs_back_entrance_shifts (day, date, shift_start_time, shift_end_time, officer)
            VALUES (%s, %s, %s, %s, %s)
            """,
            (
                entry["day"],
    ...
    db.commit()
    print("RBS Back Entrance shift data inserted successfully.")

def main():
    # Open the "Permanent Shifts" worksheet and convert to DataFrame
    sheet = gc.open_by_key(os.getenv("GOOGLE_SHEET_ID")).worksheet("RBS Back Entrance")
    all_data = sheet.get_all_values()
    df = pd.DataFrame(all_data)

    rbs_back_entrance_locations = find_rbs_back_entrance_instances(df)

    all_weeks_data = []

    for location in rbs_back_entrance_locations:
        row, col = location
        weekly_data = extract_rbs_back_entrance_data(df, row, col)
        all_weeks_data.append(weekly_data)

    rbs_back_entrance_data_df = pd.concat(all_weeks_data, ignore_index=True)

    print("Extracted RBS Back Entrance Data:")
    print(rbs_back_entrance_data_df)

    insert_missing_officers(rbs_back_entrance_data_df)

    user_input = input("Do you want to insert this data into the database? (yes/no): ")
    if user_input.lower() == "yes":
        insert_rbs_back_entrance_data(rbs_back_entrance_data_df)
    else:
