...
import os, re, pandas as pd, gspread, mysql.connector
from datetime import datetime
from dotenv import load_dotenv
...
# ─────────────────────────── Google Sheets
gc = gspread.service_account(filename=os.getenv("GOOGLE_CREDENTIALS_JSON"))
ws = gc.open_by_key(os.getenv("GOOGLE_SHEET_ID")).worksheet("PSB Lobby")
...
def extract() -> list[dict]:
    ...
    for r in range(df.shape[0]):
        for c in range(df.shape[1]):
            if str(df.iat[r, c]).strip().lower() != "psb lobby":          # B‑col
                continue

            date_row = r - 1                                              # 12/24/2024
            hour_rows  = [r + 1, r + 2]                                   # 08‑12 / 12‑17
            officer_rows = hour_rows                                      # same rows

            for col in range(c + 1, df.shape[1]):                         # iterate days
                date_raw = df.iat[date_row, col]
                if not date_raw:
                    continue
                try:
                    date_obj = datetime.strptime(date_raw, "%m/%d/%Y").date()
                except ValueError:
                    continue

                for idx, hr in enumerate(hour_rows):
                    hrs = df.iat[hr, c]                                   # "0800 - 1230"
                    if not hrs or "-" not in hrs:
                        continue
                    start, end = map(parse_time, hrs.split("-"))
                    if not start or not end:
                        continue

                    officer = clean(df.iat[officer_rows[idx], col])
                    if not officer:
                        continue

                    # ---------------------------------------------------
                    # Google‑sheet row to write into:   (hour row) + 1
                    #   hr is 0‑based DataFrame index
                    rows_out.append({
                        "shift_type"  : "PSB Lobby",
                        "date"        : date_obj,
                        "start_time"  : start,
                        ...
                        "officer_name": officer,
                        "sheet_name"  : "PSB Lobby",
                        "sheet_row"   : hr + 1,            # ← 88  / 89
                        "sheet_col"   : col + 1            # 1‑based column
                    })
    return rows_out
...
def load(rows: list[dict]):
    sql = """
        INSERT INTO shifts
         (shift_type,date,start_time,end_time,role,officer_name,
          sheet_name,sheet_row,sheet_col,status)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,'open')
        ON DUPLICATE KEY UPDATE officer_name = VALUES(officer_name)
    """
    for r in rows:
        cur.execute(sql, (
            r["shift_type"], r["date"], r["start_time"], r["end_time"],
            r["role"], r["officer_name"],
            r["sheet_name"], r["sheet_row"], r["sheet_col"]
        ))
    db.commit()
    print(f"✔  {len(rows)} rows inserted/updated.")

if __name__ == "__main__":
    data = extract()
    for d in data: print(d)
    if input("\nImport into DB?  (yes/no): ").lower() == "yes":
        load(data)
    else:
        print("Skipped.")
