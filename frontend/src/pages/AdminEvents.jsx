  ...
  async function onSubmit(e) {
    ...
    try {
      if (editingId) {
        ...
        // Step 1: Update the main event details (name, date, etc.)
        await updateEvent(editingId, {
          name: form.name,
          date: form.date,
          description: form.description,
        ...
        // Step 2: Separate existing slots from newly added slots.
        const slotsToUpdate = slots.filter(s => s.id !== null);
        const slotsToAdd = slots.filter(s => s.id === null);

        // Step 3: Send UPDATE requests for all existing slots.
        ...
        // Step 4: Send a CREATE request for all newly added slots.
    ...
    <div className="admin-events">
      ...
      <section className="admin-form">
        <div className="form-row">
            {['name','date','description','capacity'].map((key,i) => (
                <div className="input-group" key={i}>
                    <label htmlFor={`evt-${key}`}>{key.charAt(0).toUpperCase()+key.slice(1)}</label>
                    <input id={`evt-${key}`} {...(key==='date'?{type:'date'}:key==='capacity'?{type:'number'}:{type:'text'})} value={form[key]} onChange={onF(key)}/>
                </div>
            ))}
        </div>
            ...
            <table className="slots-table">
              ...
              <tbody>
                {slots.map((s,i) => (
                  <tr key={s.id || `new-${i}`}>
                    <td data-label="Officer">
                      {s.filled_name ? `${s.filled_name} (${s.filled_rank})` : '—'}
                    </td>
                    <td data-label="Assignment">
                      <input className="slot-input" placeholder="e.g. Gate" value={s.assignment} onChange={onS(i,'assignment')}/>
                    </td>
                    <td data-label="Time In">
                      <input className="slot-input" placeholder="0730" maxLength={4} value={s.time_in} onChange={onS(i,'time_in')}/>
                    </td>
                    <td data-label="Time Out">
                      <input className="slot-input" placeholder="1600" maxLength={4} value={s.time_out} onChange={onS(i,'time_out')}/>
                    </td>
                    <td className="col-action">
                      <button type="button" className="btn-icon" onClick={() => setSlots(slots.filter((_,j)=>j!==i))} aria-label="Delete slot">🗑</button>
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <tr key={ev.id}>
                ...
                <td data-label="Actions">
                  <div className="action-buttons">
                    ...
                    <button className="btn-delete" onClick={()=>onDelete(ev.id)}>Delete</button>
                  </div>
                </td>
              </tr>
            ))}
            {filtered.length===0 && (
              <tr><td colSpan="4" className="no-data">No {view} events</td></tr>
            )}
          </tbody>
        </table>
      </div>
