...
def find_civic_square_instances(df):
    """Locate all cells containing 'Civic Square' and return their row and column indices."""
    locations = []
    for row in range(df.shape[0]):
        for col in range(df.shape[1]):
            if df.iloc[row, col] == "Civic Square":
                locations.append((row, col))
    return locations

def extract_civic_square_data(df, row, col):
    """Extract data for each instance of 'Civic Square'."""
    date_row = row - 1
    day_row = row
        ...
        try:
            ...
            for j, hours_row in enumerate(hours_rows):
                ...
                if shift_hours:
                    ...
                    officer_name = df.iloc[officers_rows[j], i]
    ...
    return pd.DataFrame(civic_square_data)
...
def insert_civic_square_data(civic_square_data_df):
    """Insert the extracted Civic Square shift data into the database."""
    for _, entry in civic_square_data_df.iterrows():
        cursor.execute(
            """
            INSERT INTO civic_square_shifts (day, date, shift_start_time, shift_end_time, officer)
            VALUES (%s, %s, %s, %s, %s)
            """,
            (
                entry["day"],
    ...
    db.commit()
    print("Civic Square shift data inserted successfully.")

def main():
    # Open the "Permanent Shifts" worksheet and convert to DataFrame
    sheet = gc.open_by_key(os.getenv("GOOGLE_SHEET_ID")).worksheet("Civic Square")
    all_data = sheet.get_all_values()
    df = pd.DataFrame(all_data)

    civic_square_locations = find_civic_square_instances(df)

    all_weeks_data = []

    for location in civic_square_locations:
        row, col = location
        weekly_data = extract_civic_square_data(df, row, col)
        all_weeks_data.append(weekly_data)

    civic_square_data_df = pd.concat(all_weeks_data, ignore_index=True)

    print("Extracted Civic Square Data:")
    print(civic_square_data_df)

    insert_missing_officers(civic_square_data_df)

    user_input = input("Do you want to insert this data into the database? (yes/no): ")
    if user_input.lower() == "yes":
        insert_civic_square_data(civic_square_data_df)
    else:
